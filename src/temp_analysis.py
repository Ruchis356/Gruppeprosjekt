...
# This construct was refined with the assistance of AI 
        # AI suggested the use of "except FileNotFoundError:" code line to notify whenever files were not accessible. 
        # Source: ChatGPT
def import_for_analysis(file_path):
    try:
        with open(file_path, 'r', encoding='utf-8') as file: #The following code was constructed by AI, source: ChatGPT
            df = pd.read_csv(
                file,
                skiprows=0,  
                sep=',',    
...
# This construct was refined with the assistance of AI 
        # AI suggested the use of ".dropna()" codeline as a suggestion for calcualting weekly averages with missing weekly data.
        # Source: ChatGPT
...
# This construct was refined with the assistance of AI 
        # AI was used to suggest ways of building weekly average tables for multiple pollutants using shorter codelines
        # AI was used to fix error messages encountered while running the codes
        # Source: ChatGPT
...
sns.boxplot(data=df_air, orient='v')  
...
def find_outliers(series):
    ...
    return series[(series < lower_bound) | (series > upper_bound)] #This code was constructed by the use of AI, source: ChatGPT


# This construct was refined with the assistance of AI 
        # AI suggested how to replace values in dataframes with outliers found in the previous section
        # Source: ChatGPT
...
for col in cols_to_check:
    outliers = find_outliers(df_air[col])
    df_air_cleaned.loc[outliers.index, col] = np.nan # The following codes were generated by AI, source: ChatGPT
...
for col in cols_to_check:
    ...
    outlier_table[col] = outliers.values.tolist() #The following codes were constructed by AI, source: ChatGPT
...
for col in cols_to_check:
    outliers = find_outliers(df_weather[col])
    df_weather_cleaned.loc[outliers.index, col] = np.nan #The following codes were constructed by AI, source: ChatGPT
