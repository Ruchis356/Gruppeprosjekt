...
__all__ = ['Graphs', 'PredictiveGraphs']  
...
class Graphs:
    """Creates time-series visualizations of environmental data.
    
    Features:
    - Scatter plots with date handling
    - Dual-axis comparative plots
    - Consistent styling throughout
    
    Note: Uses matplotlib/seaborn with AI-assisted layout optimizations.
    """
    ...
    def dot_graph(self, df, columns, title, x_axis, y_axis, color=None):
        ...
        for i, column in enumerate(columns):
            ...
            sns.scatterplot(data=df, x='Date', y=column,  # This line was generated by AI (DeepSeek)
                            label=column, 
                            color=plot_color,
                            alpha=0.5)
...
class PredictiveGraphs:
    ...
    def plot_full_overview(self, historical_df, forecast_temp, forecast_precip, forecast_wind, api_df):
        ...
        historical_req = ['Date', 'temperature (C)', 'wind_speed (m/s)', 'precipitation (mm)']
        forecast_req = ['Date', 'Prediction']
        api_req = ['Date', 'temperature (C)', 'wind_speed (m/s)', 'precipitation (mm)']

        missing = {
            'historical_df': [col for col in historical_req if col not in historical_df],
            'forecast_temp': [col for col in forecast_req if col not in forecast_temp],
            'forecast_precip': [col for col in forecast_req if col not in forecast_precip],
            'forecast_wind': [col for col in forecast_req if col not in forecast_wind],
            'api_df': [col for col in api_req if col not in api_df]
        }

        errors = [f"{name}: Missing {cols}" for name, cols in missing.items() if cols]
        if errors:
    ...
    def plot_week_comparison(self, forecast_temp, forecast_precip, 
        ...
        self.plot_full_overview(
            pd.DataFrame(columns=['Date', 'temperature (C)', 'wind_speed (m/s)', 'precipitation (mm)']),  # Dummy
            forecast_temp,
            forecast_precip,
            forecast_wind,
            api_df
    ...
    def plot_results(self, test_data, target):
        ...
        if 'Prediction' not in test_data.columns:
            raise ValueError("test_data must contain 'Prediction' column")
    ...
    def plot_pollutant_forecasts(self, forecasts, pollutants=None):
        ...
        if not isinstance(forecasts, dict):
            raise TypeError("forecasts must be a dictionary")
        if not forecasts:
            raise ValueError("forecasts dictionary is empty")
        
        # Validate each forecast DataFrame
        for poll, df in forecasts.items():
            if not isinstance(df, pd.DataFrame):
                raise TypeError(f"forecasts['{poll}'] must be a DataFrame")
            if 'Date' not in df.columns or f'{poll}_forecast' not in df.columns:
                raise ValueError(f"forecasts['{poll}'] must contain 'Date' and '{poll}_forecast' columns")
        
        # Validate pollutants list
        if pollutants is not None:
            if not isinstance(pollutants, list):
                raise TypeError("pollutants must be a list or None")
            missing = [p for p in pollutants if p not in forecasts]
            if missing:
                raise ValueError(f"Pollutants not in forecasts: {missing}")
    ...
    def model_information(self, model, merged_data, weather_vars, target, features):
        ...
        # Validate DataFrames/columns
        if not isinstance(merged_data, pd.DataFrame):
            raise TypeError("merged_data must be a DataFrame")
        
        missing = {
            'weather_vars': [var for var in weather_vars if var not in merged_data],
            'target': [target] if target not in merged_data else [],
            'features': [feat for feat in features if feat not in merged_data]
        }
        
        errors = [f"Missing in merged_data: {k}={v}" for k, v in missing.items() if v]
        if errors:
            raise ValueError("\n".join(errors))
